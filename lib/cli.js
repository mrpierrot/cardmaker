...
const { build, fetch, setup, watch, exportToPdf } = require('./index');
...
function helpCmd(options = {}) {
    ...
    const sections = {
        undefined: [
            ...
            },
            {
                ...
                content: [
                    ...
                    { name: 'export', summary: 'Export cards to pdf.' }
                ]
            }
        ],
        fetch: [
        ...
        export: [
            {
                header: 'export',
                content: 'Export cards to pdf.'
            },
            {
                header: 'Synopsis',
                content: 'cardmaker export <options>'
            },
            {
                header: 'Options',
                optionList: [
...
function exportToPdfCmd(options) {
    return getConf(currentPath)
        .then(conf => {
            log.info(`try to execute "export" command`);
            const { templates, layouts, output, gsheet } = conf;

            exportToPdf({ templates, layouts, currentLayout: options.layout, output, gsheet, currentPath })
                .catch(err => log.error(`Error: ${err.message}`))
                .then(() => log.strong(`export over`));
...
const validCommands = ['build', 'setup', 'help', 'fetch', 'watch', 'export'];
...
getCommandLine(validCommands).then(({ command, argv }) => {
    ...
    switch (command) {
        ...
        case 'export': exportToPdfCmd(options); break;
        default:
